...
import { projectTemplatesDirPath } from '../utils';
import { ProjectEntity } from '../projects';

@Injectable()
export class PagesExportStaticHtmlService {
  constructor(
  ...
  async exportAndZip(id: number): Promise<DownloadableFileDetails> {
    ...
    const fileName = `${page.project.name}${page.path ? `-${page.path}` : ''}-(static-html).zip`;
  ...
  async export(
    ...
    outDirPath: string,
    options?: { assetsOutDir?: string; assetsPrefixPath?: string },
  ): Promise<void> {
    ...
    websolutManager.export({
      outDirPath,
      templatesDirPath,
      assetsOutDir: options?.assetsOutDir,
      assetsPrefixPath: options?.assetsPrefixPath,
    });
  }
}
