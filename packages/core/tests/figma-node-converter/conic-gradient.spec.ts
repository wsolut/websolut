...
describe('NodeWrapper', () => {
  describe('conic gradients', () => {
    ...
    it('should emit conic-gradient with stops and center when 2+ stops', () => {
      const instance = FigmaNodeConverter.create({
        ...FigmaExamples.frame,
        fills: [
          {
      ...
      expect(img).toMatch(/^conic-gradient\(/);
    ...
    it('should mix with images and linear/radial gradients in layer order', () => {
      const instance = FigmaNodeConverter.create({
        ...FigmaExamples.frame,
        fills: [
          {
      ...
      const img = instance.cssBackgroundImage();
      expect(img).toMatch(
        /^url\(|^url\(.*\),\s*linear-gradient\(.*\),\s*radial-gradient\(.*\),\s*conic-gradient\(/,
      );

      const blend = instance.cssBackgroundBlendMode();
      expect(blend).toBe('overlay, normal, color-dodge, linear-burn');
    });
  });
});
